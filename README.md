...
https://github.com/arceos-org/arceos/pull/267
![](pic/incorrect-writev-fix.png)
这个修改是针对原代码假设每次 sys_write 必然完全成功，且未处理：系统调用返回错误部分写入​​（实际写入字节数 < 请求字节数）

新实现符合标准​​：
修改后的逻辑严格遵循了 UNIX writev(2) 系统调用的规范：
遇到错误​​立即中止​​并返回错误码
遇到部分写入时​​保留已成功写入的数据量​​并返回
最终返回值 ret 精确表示​​实际写入的总字节数​


https://github.com/arceos-org/arceos/pull/268
![](pic/add_read-to-end.png)
主要是增加了一个一次性读入到缓冲区的功能
## 总结与收获
